            ...
            <tbody class="text-sm divide-y divide-gray-50 dark:divide-slate-700/50">
                ...
                <tr class="hover:bg-white/80 dark:bg-slate-800/80 transition-colors">
                    ...
                    <td class="p-4 text-right">
                        <div class="inline-block text-left relative" x-data="{ openMenu: false }">
                            <button type="button" @click="openMenu = !openMenu" @click.away="openMenu = false"
                                    class="w-8 h-8 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-700/60 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 transition-all border border-slate-200/80 dark:border-slate-700 inline-flex items-center justify-center"
                                    title="Pilihan Aksi">
                                <i class="ph ph-dots-three-vertical text-lg"></i>
                            </button>

                            <div x-show="openMenu" x-transition.opacity.duration.150ms
                                 class="absolute right-0 mt-2 w-48 bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-gray-100 dark:border-slate-700 py-1.5 z-40 text-xs font-medium divide-y divide-gray-100 dark:divide-slate-700/50"
                                 style="display: none;">
                                
                                <div class="py-1">
                                    <!-- Atur Layanan / Langganan -->
                                    <button type="button"
                                        @click="openMenu = false; openSubModal(<?= (int)$u['id'] ?>, <?= htmlspecialchars(json_encode($u['name']), ENT_QUOTES, 'UTF-8') ?>, '<?= (int)($u['sub_plan_id'] ?? 1) ?>', '<?= htmlspecialchars($u['sub_expires'] ?? '') ?>')"
                                        class="w-full flex items-center gap-2.5 px-3.5 py-2 text-slate-700 dark:text-slate-200 hover:bg-sky-50 dark:hover:bg-sky-900/30 hover:text-sky-600 dark:hover:text-sky-300 transition-colors">
                                        <i class="ph ph-calendar-plus text-base text-sky-500"></i> Atur Layanan / Expired
                                    </button>

                                    <!-- Ganti Password -->
                                    <button type="button"
                                        @click="openMenu = false; openPasswordModal(<?= (int)$u['id'] ?>, <?= htmlspecialchars(json_encode($u['name']), ENT_QUOTES, 'UTF-8') ?>)"
                                        class="w-full flex items-center gap-2.5 px-3.5 py-2 text-slate-700 dark:text-slate-200 hover:bg-amber-50 dark:hover:bg-amber-900/30 hover:text-amber-600 dark:hover:text-amber-300 transition-colors">
                                        <i class="ph ph-key text-base text-amber-500"></i> Ganti Password
                                    </button>
                                </div>
                                ...
                                <div class="py-1">
                                    <?php if ($isAdminRole): ?>
                                        <button type="button" 
                                            @click="openMenu = false; openConfirm('demote', <?= (int)$u['id'] ?>, 'Cabut Status Admin?', <?= htmlspecialchars(json_encode('Cabut hak akses admin dari ' . $u['name'] . '?'), ENT_QUOTES, 'UTF-8') ?>, 'Ya, Cabut Admin', 'bg-slate-700 hover:bg-slate-800 text-white', 'ph-user-minus text-slate-600 dark:text-slate-300', 'bg-slate-100 dark:bg-slate-700')"
                                            class="w-full flex items-center gap-2.5 px-3.5 py-2 text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors">
                                            <i class="ph ph-user-minus text-base text-slate-500"></i> Cabut Status Admin
                                        </button>
                                    <?php else: ?>
                                        <button type="button"
                                            @click="openMenu = false; openConfirm('promote', <?= (int)$u['id'] ?>, 'Jadikan Admin?', <?= htmlspecialchars(json_encode('Jadikan ' . $u['name'] . ' sebagai admin dengan akses penuh ke Panel Admin PahamFin?'), ENT_QUOTES, 'UTF-8') ?>, 'Ya, Jadikan Admin', 'bg-amber-600 hover:bg-amber-700 text-white', 'ph-shield-plus text-amber-600 dark:text-amber-400', 'bg-amber-100 dark:bg-amber-900/40')"
                                            class="w-full flex items-center gap-2.5 px-3.5 py-2 text-slate-700 dark:text-slate-200 hover:bg-amber-50 dark:hover:bg-amber-900/30 hover:text-amber-600 dark:hover:text-amber-300 transition-colors">
                                            <i class="ph ph-shield-plus text-base text-amber-500"></i> Jadikan Admin
                                        </button>
                                    <?php endif; ?>

                                    <button type="button"
                                        @click="openMenu = false; openConfirm('delete', <?= (int)$u['id'] ?>, <?= htmlspecialchars(json_encode('Hapus ' . $u['name'] . '?'), ENT_QUOTES, 'UTF-8') ?>, <?= htmlspecialchars(json_encode('Hapus ' . $u['name'] . ' beserta semua data transaksinya? Aksi ini tidak dapat dibatalkan.'), ENT_QUOTES, 'UTF-8') ?>, 'Ya, Hapus Pengguna', 'bg-rose-600 hover:bg-rose-700 text-white', 'ph-trash text-rose-600 dark:text-rose-400', 'bg-rose-100 dark:bg-rose-900/40')"
                                        class="w-full flex items-center gap-2.5 px-3.5 py-2 text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/40 transition-colors">
                                        <i class="ph ph-trash text-base text-rose-500"></i> Hapus Akun
                                    </button>
                                </div>
                                <?php endif; ?>
                            </div>
